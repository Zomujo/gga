...
export function DashboardNav({
  ...
  return (
    <nav className="border-b border-[#d7c8ab]/70 bg-[#fbfaf7] shadow-sm">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6">
          ...
          <div className="ml-auto flex shrink-0 items-center gap-3 sm:gap-4">
            ...
            {(isAdmin || isNavigator) && !isDistrictOfficer && (
              <div className="hidden h-10 w-auto items-center justify-center overflow-hidden sm:flex">
                <button
                  onClick={onNewCase}
                  className="rounded-lg bg-[#7a5a3b] px-3 py-2 text-sm font-semibold text-white hover:bg-[#62482f] sm:px-4"
                >
                  + New Report
                </button>
              </div>
          ...
          <div className="space-y-3 sm:hidden">
            {isSuperAdmin && districtOptions.length > 0 && onDistrictChange && (
              <label className="block">
                ...
                <select
                  className="w-full rounded-lg border border-[#cdbd9a] bg-white px-3 py-2 text-sm focus:border-[#7a5a3b] focus:outline-none"
                  value={selectedDistrict || ""}
                  onChange={(e) => onDistrictChange(e.target.value)}
                >
                  {districtOptions.map((option) => (
            ...
            <button
              onClick={onNewCase}
              className="w-full rounded-lg bg-[#7a5a3b] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#62482f]"
            >
              + New Report
            </button>
          </div>
        ...
        {(isAdmin || isNavigator || isDistrictOfficer) && (
          <div className="-mx-1 overflow-x-auto lg:mx-0">
            <div className="mx-auto flex w-max min-w-max gap-2 rounded-lg bg-[#ece5d8] p-1">
              {visibleTabs.map((tab) => (
                <button
                  key={tab.id}
                  ...
                  className={`shrink-0 rounded-md px-3 py-2 text-sm font-medium whitespace-nowrap transition-colors sm:px-4 ${
                    activeTab === tab.id
                      ? "bg-[#7a5a3b] text-white shadow-sm"
